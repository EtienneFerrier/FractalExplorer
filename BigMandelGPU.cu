/*
Ce fichier implémente le calcul de l'ensemble de Mandelbrot sur GPU avec precision arbitraire
*/
...
// Affiche un grand flottant en écriture x.10^n en utilisant les 2 premieres decimales uniquement.
...
// Affiche un grand flottant de maniere brute.
...
// Meme affiche que display2dec mais sur un tableau de grands flottants.
...
// Convertit une couleur RGB en Uint32
__device__ uint32_t couleur(int r, int g, int b)
{
	return (((r << 8) + g) << 8) + b;
}

// Methode de coloration de fractale
...
// Charge 1/WIDTH dans le grand flottant
...
/*
tmpLittle est un tas qui contient la somme partielle a ajouter plus tard a little
tmpBig est un tas qui contient la somme partielle a ajouter plus tard a big
carry est un tas qui contient la somme des retenues a ajouter plus tard au decimal avant big
*/
...
__device__ bool testSquare(bool* posX, uint32_t* decX, bool* posY, uint32_t* decY)
{
	...
	multIP(posX, decX, *posX, decX);	// X = x^2
	multIP(posY, decY, *posY, decY);	// Y = y^2
	addIP(posX, decX, *posY, decY);		// X = x^2 + y^2
...
// Renvoie les couleurs calculees dans le tableau res.
...
// Fonction principale d'allocation de memoire
